...
document.getElementById('step2Form').addEventListener('submit', function(event) {
    ...
    for (var i = 0; i < cgpaInputs.length; i++) {
        ...
        if (isNaN(cgpa) || cgpa < 1.00 || cgpa > 4.00) {
            alert('Please enter a valid CGPA between 1.00 and 4.00 for all courses.');
            return;
        }
        totalCGPA += cgpa;
        if (cgpa < 2.00) {
    ...
    if (failedCourses.length > 0) {
        resultMessage += 'You have been failed in course ' + failedCourses.join(' & ') + '<br>';
    }
